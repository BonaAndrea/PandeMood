...
public class FlyBehaviour : GenericBehaviour
{
	...
	public float tempoInVolo;
	private float timeFlight;
	public float fattoreRefrattario;
	...
	void Update()
	{
		...
		//ricarica il tempo in volo
        if (!fly && tempoInVolo < timeFlight)
        {
			tempoInVolo += Time.deltaTime*fattoreRefrattario;
			if (tempoInVolo >= timeFlight)
				tempoInVolo = timeFlight;
        }
	}
	...
	Vector3 Rotating(float horizontal, float vertical)
	{
		...
		Vector3 forward = BehaviourManager.playerCamera.TransformDirection(Vector3.forward);
		//Debug.Log(behaviourManager.playerCamera.TransformDirection(Vector3.forward));
