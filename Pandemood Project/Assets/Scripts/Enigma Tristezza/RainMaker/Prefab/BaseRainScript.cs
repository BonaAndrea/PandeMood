...
namespace DigitalRuby.RainMaker
{
    ...
    {
        //*****ADDED FUNCTION FOR TIMING RAIN*********
        [Tooltip("Time to start raining")]
        public float timeToStartRain = 0f;

        [Tooltip("Time to end raining")]
        public float timeToEndRain = 0f;
        //***********************
        ...
        //*****ADDED FUNCTION FOR TIMING RAIN*********
        public bool start_rain = false;
        public bool end_rain = false;

        private IEnumerator StartRain()
        {
            yield return new WaitForSeconds(timeToStartRain);
            start_rain = true;
            StartCoroutine(StartRaining());
        }

        //OGNI 3 SECONDI AUMENTA L'INTENSITA DELLA PIOGGIA DI 0.1
        ...
        {
            yield return new WaitForSeconds(timeToEndRain);
            end_rain = true;
            StartCoroutine(EndRaining());
        }

        //OGNI 0.5 SECONDI DIMINUISCE L'INTENSITA DELLA PIOGGIA DI 0.1
        private IEnumerator EndRaining()
        {
